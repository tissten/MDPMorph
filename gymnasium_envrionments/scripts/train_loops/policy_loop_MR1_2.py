...
#####################################################################################################33
            ...
            rand_value = 0.05
            gam =  np.full(13, rand_value)  # 生成前6个元素的扰动
            ...
            for eval_episode_counter in range(number_eval_episodes):
                ...
                distance = np.linalg.norm(denormalised_action2 - denormalised_action1)
                ...
                if flag == 0:
                    ...
                    if eval_episode_counter %2000 == 0:
                        output_file = os.path.join("./Metamorphic/training_threshold", "MR1.2_threshold_FalsePositive_test_case.csv")
        ...
        if flag == 0:
            output_file = os.path.join("./Metamorphic/training_threshold", "MR1.2_threshold_FalsePositive_test_suite.csv")
            with open(output_file, mode='a', newline='') as F:
                writer = csv.writer(F)
                writer.writerow([para])
