...
#####################################################################################################33
            ...
            for eval_episode_counter in range(number_eval_episodes):
                ...
                distance = np.linalg.norm(np.array(total_reward1) - np.array(total_reward2))
                ...
                if flag == 0:
                    ...
                    if eval_episode_counter %20 == 0:
                        output_file = os.path.join(r"F:\UoA tool paper\MDPMorph_open_source\Metamorphic\training_threshold", "MR5_threshold_FalsePositive_test_case.csv")
        ...
        if flag == 0:
            output_file = os.path.join(r"F:\UoA tool paper\MDPMorph_open_source\Metamorphic\training_threshold", "MR5_threshold_FalsePositive_test_suite.csv")
            with open(output_file, mode='a', newline='') as F:
                writer = csv.writer(F)
                writer.writerow([para])
