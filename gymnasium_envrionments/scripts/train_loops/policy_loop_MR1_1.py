...
def evaluate_policy_network(
    ...
    ###########################################################
    file_names = [rf"./Metamorphic/test suite\T{i}.txt" for i in range(40)]
    para = 0
...
#####################################################################################################33
            ...
            noise = np.random.normal(loc=0, scale=0.1, size=13)  # 生成前6个元素的扰动
            ...
            for eval_episode_counter in range(number_eval_episodes):
                ...
                episode_num += 1
                        # break
                distance = np.linalg.norm(denormalised_action2 - denormalised_action2)
                # distance = np.sum(np.linalg.norm(np.array(total_state1) - np.array(total_state2), axis=1))
                ...
                if flag == 0:
                    ...
                    print(distance)
                    if eval_episode_counter %2000 == 0:
                        output_file = os.path.join("./Metamorphic/training_threshold", "MR1.1_threshold_FalsePositive_test_case.csv")
        ...
        if flag == 0:
            output_file = os.path.join("./Metamorphic/training_threshold", "MR1.1_threshold_FalsePositive_test_suite.csv")
            with open(output_file, mode='a', newline='') as F:
                writer = csv.writer(F)
                writer.writerow([para])
