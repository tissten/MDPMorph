...
def evaluate_policy_network(
    env, agent1, agent2, config: TrainingConfig, record=None, total_steps=0, normalisation=True
):
    
    ###########################################################
    file_names = [f"/home/jil824/CARES_NEW/Metamorphic/test suite/T{i}({i * 10}).txt" for i in range(40, 50)]
...
#####################################################################################################33
            ...
            for eval_episode_counter in range(number_eval_episodes):
                ...
                while not done and not truncated:
                    ...
                    # print("1")
                    state1, reward1, done, truncated = env.step(denormalised_action1)
                    state2, reward2, done, truncated = env.step(denormalised_action2)
