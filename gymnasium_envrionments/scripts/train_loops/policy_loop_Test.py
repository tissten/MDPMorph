...
def evaluate_policy_network(
    ...
    ###########################################################
    file_names = [f"./Metamorphic/test suite\T{i}({i * 10}).txt" for i in range(40, 41)]
    # file_names = [f"/home/jil824/CARES_NEW/Metamorphic/test suite/T{i}({i * 10}).txt" for i in range(42, 43)]
    # para = 0
...
#####################################################################################################33
            ...
            for eval_episode_counter in range(number_eval_episodes):
                ...
                # reward_sum1 = sum(total_reward1[:70]) if len(total_reward1) >= 70 else sum(total_reward1)
                # reward_sum2 = sum(total_reward1[:10]) if len(total_reward1) >= 10 else sum(total_reward1)
                # distance = reward_sum1 - reward_sum2
                # if distance > 15.9:
                #     MR += 1
                #     print(distance)
        ...
        output_file4 = os.path.join("./Metamorphic/final state", f'Completion Rate_BipedalWalker.txt')
                
        with open(output_file4, "a") as f:
                f.write(str(Model_name+1) + ": " + str(number/100) + "\n")
